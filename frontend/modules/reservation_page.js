 ...
 if(reservations.length==0){
  ...
  reservations.forEach(element => {
    ...
    const actionBtn = `<a href="../detail/?adventure=${adventureId}" ><button class='reservation-visit-button'>Visit Adventure</button></a>`
     ...
     const text= new Date(d).toLocaleDateString('en-IN', { year: 'numeric', month: 'long', day: 'numeric', hour:'numeric', 
    minute: 'numeric', second:'numeric', hour12: true })
     ...
     <td>${bookingData}</td>
     <td>${actionBtn}</td>`
    document.getElementById("no-reservation-banner").appendChild(row);
